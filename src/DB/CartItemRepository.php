...
class CartItemRepository extends \StORM\Repository
{
	...
	public function updateCartItemAmount(CartItem $cartItem, Product $product, int $amount): int
	{
		return $this->many()
			...
			->update(['originalAmount' => $amount, 'changeAmountReason' => null, 'amount' => $amount, 'price' => $product->getPrice($amount), 'priceVat' => $product->getPriceVat($amount)]);
	}
