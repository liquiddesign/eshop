		...
		foreach (\array_keys($this->getItemsOnPage()) as $key) {
			...
			if (isset($openingHours[$key]['special'][$currentDate]->openFrom)) {
				...
				$openingHoursTexts[$key] .= isset($openingHours[$key]['special'][$currentDate]->openTo) ?
					' ' . $this->translator->translate('pickupPointList.to', 'do') . ' ' . \substr($openingHours[$key]['special'][$currentDate]->openTo, 0, -3) : '';
				$openingHoursTexts[$key] .= isset($openingHours[$key]['special'][$currentDate]->pauseFrom)
					? '<br>(' . $this->translator->translate('pickupPointList.pauseFrom', 'pauza od') . ' ' . \substr($openingHours[$key]['special'][$currentDate]->pauseFrom, 0, -3) : '';
				$openingHoursTexts[$key] .= isset($openingHours[$key]['special'][$currentDate]->pauseTo)
					? ' ' . $this->translator->translate('pickupPointList.to', 'do') . ' ' . \substr($openingHours[$key]['special'][$currentDate]->pauseTo, 0, -3) . ')' : '';
				...
				$openingHoursTexts[$key] .= isset($openingHours[$key]['normal'][$weekDay]->openTo) ?
					' ' . $this->translator->translate('pickupPointList.to', 'do') . ' ' . \substr($openingHours[$key]['normal'][$weekDay]->openTo, 0, -3) : '';
				$openingHoursTexts[$key] .= isset($openingHours[$key]['normal'][$weekDay]->pauseFrom) ?
					'<br>(' . $this->translator->translate('pickupPointList.pauseFrom', 'pauza od') . ' ' . \substr($openingHours[$key]['normal'][$weekDay]->pauseFrom, 0, -3) : '';
				$openingHoursTexts[$key] .= isset($openingHours[$key]['normal'][$weekDay]->pauseTo) ?
					' ' . $this->translator->translate('pickupPointList.to', 'do') . ' ' . \substr($openingHours[$key]['normal'][$weekDay]->pauseTo, 0, -3) . ')' : '';
			} else {
