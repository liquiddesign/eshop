...
class CartItemWithFormattedPrices
{
	...
	{
		return $this->getProduct() && $this->getProduct()->inStock();
	}

	public function getDisplayAmountText(): ?string
	{
		if ($this->getProduct() && $this->getProduct()->displayAmount) {
			return $this->getProduct()->displayAmount->label;
		}
	...
	{
		if ($this->inStock()) {
			return ($text = $this->getProduct()->getDynamicDelivery()) ?
				$text :
	...
	public function getStorageDateText(): ?string
	{
		...
		return $this->getProduct() && $this->getProduct()->storageDate ?: $this->translator->translate('.storageUnknown', 'Naskladnění neznámé');
	}
