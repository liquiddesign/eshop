...
class ProductsCacheWarmUpService implements AutoWireService
{
	...
	public function warmUpCacheTableDiff(): void
	{
		...
		try {
			...
			Debugger::dump('dropCategoriesTables: ' . Debugger::timer() . ', ' . DevelTools::getPeakMemoryUsage());
			...
			Debugger::dump('warmUpRelations: ' . Debugger::timer() . ', ' . DevelTools::getPeakMemoryUsage());
	...
	protected function diffUpdateMainTable(
		...
		foreach ($allCategoryTypes as $categoryType) {
			...
			$this->getLink()->exec("ALTER TABLE $productsCacheTableName ADD primaryCategory_{$categoryType->id} INT UNSIGNED;");
			$this->getLink()->exec("ALTER TABLE `$productsCacheTableName` ADD INDEX idx_primaryCategory_{$categoryType->id} (primaryCategory_{$categoryType->id});");
		}

		foreach ($currentColumns as $currentColumn) {
			$this->getLink()->exec("ALTER TABLE `$productsCacheTableName` DROP INDEX `idx_$currentColumn`;");
			$this->getLink()->exec("ALTER TABLE `$productsCacheTableName` DROP COLUMN `$currentColumn`");
		}
