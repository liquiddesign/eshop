...
//		$form->setDefaults(['products' => $products]);
	...
	{
		if ($pricelist->isReadonly) {
			$this->flashMessage('Ceník je pouze pro čtení, nelze přidávat nové ceny.', 'error');
			$this->redirect('priceListItems', $pricelist);
		}
	...
	public function createComponentAggregateForm(): AdminForm
	{
		...
		$targetPricelistInput = $form->addDataSelect(
			'targetPricelist',
			'Cílový ceník',
			$this->priceListRepository->many()->where('isReadonly', false)->toArrayOf('name'),
		);
