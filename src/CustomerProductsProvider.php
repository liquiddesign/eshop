...
CREATE TABLE `$productsCacheTableName` (
  ...
  customerGroup INT UNSIGNED,
  visibilityList INT UNSIGNED,
  hidden TINYINT,
  hiddenInMenu TINYINT,
  priority SMALLINT,
  unavailable TINYINT,
  recommended TINYINT,
  price DOUBLE,
  ...
  INDEX idx_customerGroup (customerGroup),
  FULLTEXT INDEX idx_name (name)
  
);");
			$link->exec("CREATE UNIQUE INDEX idx_unique_code_customer ON `$productsCacheTableName` (code, customer);");
			$link->exec("CREATE UNIQUE INDEX idx_unique_code_group ON `$productsCacheTableName` (code, customerGroup);");
			$link->exec("CREATE UNIQUE INDEX idx_unique_ean_customer ON `$productsCacheTableName` (ean, customer);");
			$link->exec("CREATE UNIQUE INDEX idx_unique_ean_group ON `$productsCacheTableName` (ean, customerGroup);");
			$link->exec("CREATE UNIQUE INDEX idx_unique_product_customer ON `$productsCacheTableName` (product, customer);");
			$link->exec("CREATE UNIQUE INDEX idx_unique_product_group ON `$productsCacheTableName` (product, customerGroup);");

//			$allVisibilityLists = $this->visibilityListRepository->many()->select(['this.id'])->setIndex('this.id')->fetchArray(\stdClass::class);
...
//			$allPriceLists = $this->pricelistRepository->many()->select(['this.id'])->setIndex('this.id')->fetchArray(\stdClass::class);
...
//				], keepIndex: true)->fetchArray(\stdClass::class);
			...
			->setSelect([
				...
				'ean' => 'this.ean',
				'pricesIDs' => 'GROUP_CONCAT(DISTINCT price.id ORDER BY priceList.priority)',
				'categoriesPKs' => 'GROUP_CONCAT(DISTINCT eshop_product_nxn_eshop_category.fk_category)',
				'visibilityListItemsIDs' => 'GROUP_CONCAT(DISTINCT visibilityListItem.id ORDER BY visibilityList.priority)',
			...
			$customers = $this->customerRepository->many()
				->setGroupBy(['this.id'])
				->setIndex('this.id')
				->join(['eshop_customer_nxn_eshop_visibilitylist'], 'this.uuid = eshop_customer_nxn_eshop_visibilitylist.fk_customer')
				->join(['eshop_customer_nxn_eshop_pricelist'], 'this.uuid = eshop_customer_nxn_eshop_pricelist.fk_customer')
				->join(['visibilityList' => 'eshop_visibilitylist'], 'eshop_customer_nxn_eshop_visibilitylist.fk_visibilityList = visibilityList.uuid')
				->join(['priceList' => 'eshop_pricelist'], 'eshop_customer_nxn_eshop_pricelist.fk_pricelist = priceList.uuid')
				->select([
					'id' => 'this.id',
					'visibilityListsIDs' => 'GROUP_CONCAT(DISTINCT visibilityList.id ORDER BY visibilityList.priority)',
					'priceListsIDs' => 'GROUP_CONCAT(DISTINCT priceList.id ORDER BY priceList.priority)',
				])
				->toArray();

//			$customerGroups = $this->customerGroupRepository->many()->select(['id' => 'this.id'])->toArray();

			while ($product = $productsCollection->fetch(\stdClass::class)) {
				/** @var \stdClass $product */

//				if (!$prices = $product->pricesIDs) {
//					continue;
//				}
				...
				if ($categories = $product->categoriesPKs) {
					...
					foreach ($categories as $category) {
						...
						if ($categoryCategories === null) {
						 ...
						 $productArray['categories'] = \array_unique(\array_merge($productArray['categories'] ?? [], $categoryCategories));
				...
				$productArray['attributeValues'] = $product->attributeValuesPKs;

				$visibilityListItemIDs = $product->visibilityListItemsIDs ? \explode(',', $product->visibilityListItemsIDs) : [];
				$productVisibilityListItems = [];

				foreach ($visibilityListItemIDs as $visibilityListItemID) {
					$visibilityListItem = $allVisibilityListItems[$visibilityListItemID];

					$productVisibilityListItems[$visibilityListItem->visibilityListId] = $visibilityListItem;
				}

				foreach ($customers as $customer) {
					$customerProduct = $productArray;

					$customerProduct['customer'] = $customer->getValue('id');

					$customerVisibilityListsIDs = $customer->getValue('visibilityListsIDs') ? \explode(',', $customer->getValue('visibilityListsIDs')) : [];
					$currentVisibilityLists = \array_intersect($customerVisibilityListsIDs, \array_keys($productVisibilityListItems));

					$customerProduct['visibilityList'] = null;
					$customerProduct['hidden'] = null;
					$customerProduct['hiddenInMenu'] = null;
					$customerProduct['priority'] = null;
					$customerProduct['unavailable'] = null;
					$customerProduct['recommended'] = null;

					foreach ($currentVisibilityLists as $visibilityListID) {
						$productVisibilityListItem = $productVisibilityListItems[$visibilityListID];

						$customerProduct['visibilityList'] = $visibilityListID;
						$customerProduct['hidden'] = $productVisibilityListItem->hidden;
						$customerProduct['hiddenInMenu'] = $productVisibilityListItem->hiddenInMenu;
						$customerProduct['priority'] = $productVisibilityListItem->priority;
						$customerProduct['unavailable'] = $productVisibilityListItem->unavailable;
						$customerProduct['recommended'] = $productVisibilityListItem->recommended;
